...
//                                                                                                                        ║
//                                                         Sprites                                                        ║
#define mod_jaws 3 // Jaws: 3 - beautiful; 2 - standart; 1 - thin; 0 - type0;                                             ║4  options ╗
...
#define mod_nose 5 // Nostrils: 7-triangle; 6-heart; 5-V; 4-dots1; 3-dots2; 2-dots3; 1-dots4; 0-dots5;                    ║8  options ║
...
int cnt_kl = 9;    // Number of sensor touches for death mode                                                             ║
...
Max72xxPanel matrix = Max72xxPanel(9, 1, 11); //matrix connection, only I allowed to touch 
...
void setup()
{
  ...
  matrix.setRotation(4); // Sprite Direction  1,2,3,4 // 4 - the most common, if it does not work, then install 3 and change "Max72xxPanel matrix = Max72xxPanel(9, 1, 11);" on "Max72xxPanel matrix = Max72xxPanel(9, 11, 1);"
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Sprite Output 
  ...
  {
    for ( m1 = 0; m1 < 4; m1++){
    for (int y = 72; y < 88; y++){matrix.drawPixel(7-m1, y, LOW);}
    for (int y = 72; y < 88; y++){matrix.drawPixel(0+m1, y, LOW);}
    for (int y = 48; y < 64; y++) {matrix.drawPixel(7-m1, y, LOW);}
    for (int y = 48; y < 64; y++) {matrix.drawPixel(0+m1, y, LOW);}
    matrix.write(); 
    delay(t);
    }
    for ( m1 = 0; m1 < 4; m1++){
    for (int y = 0; y < 16;y++){
        matrix.drawPixel(3-m1, y+72, eyes[y] & (1 << 3-m1));
        matrix.drawPixel(3-m1, 63-y, eyes[y] & (1 << 3-m1));
        matrix.drawPixel(4+m1, y+72, eyes[y] & (1 << 4+m1));
        matrix.drawPixel(4+m1, 63-y, eyes[y] & (1 << 4+m1));
    }
    matrix.write(); 
    delay(t);    
    }
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
  ...
  if (nos == HIGH){
    for (int y = 0; y < 8; y++) {
      for (int x = 8; x >= 0; x--) {
        matrix.drawPixel(x, y+64, nose[y] & (1 << x));
    }}matrix.write();
  }
  if (nos == LOW){
    for (int y = 8; y < 16; y++) {
      for (int x = 8; x >= 0; x--) {
        matrix.drawPixel(x, y+56, nose[y] & (1 << x));
    }}matrix.write();
  }
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
 ...
 if (smile  == HIGH & zloy  == LOW & kill  == LOW){
    ...
    for (int y = 0+16*sprite_smile; y < 16+16*sprite_smile; y++) {
      for (int x = 8; x >= 0; x--) {
        matrix.drawPixel(x, y+72-16*sprite_smile, owo_eyes[y] & (1 << x));
        matrix.drawPixel(x, 63+16*sprite_smile-y, owo_eyes[y] & (1 << x));
    }
    }
    }
    else{
    for (int y = 0; y < 16; y++) {
      for (int x = 8; x >= 0; x--) {
        matrix.drawPixel(x, y+72, owo_eyes[y] & (1 << x));
        matrix.drawPixel(x, 63-y, owo_eyes[y] & (1 << x));
    }
    }
    }
    if (alt_owo != 0){
          for (int y = 0; y < 24; y++){
            for (int x = 8; x >= 0; x--){
              ...
              matrix.drawPixel(x, 47-y, o[y] & (1 << x));
    }
    }
    matrix.write();
    }
        else{
      for (int y = 24*rot; y < 24+24*rot; y++){
        for (int x = 8; x >= 0; x--){
          matrix.drawPixel(x, y-24*rot, jaws[y] & (1 << x));
          matrix.drawPixel(x, 47+24*rot-y, jaws[y] & (1 << x));
      }
    }
    matrix.write();
    }
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
  ...
  if (kill == 1){   
    ...
    for (int y = 0; y < 16; y++){
      for (int x = 8; x >= 0; x--){
          matrix.drawPixel(x, y+72, dead_eyes[y] & (1 << x));
          matrix.drawPixel(x, 63-y, dead_eyes[y] & (1 << x));
      }
    }    
  matrix.write();
  }
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
  ...
  if (zloy  == HIGH & kill == 0){
    if (alt_ang  != 0){
      for (int y = 0; y < 24; y++){
        for (int x = 8; x >= 0; x--){
          ...
          matrix.drawPixel(x, 47-y, z[y] & (1 << x));
      }
    }
    matrix.write();
    }
    ...
    for (int y = 0; y < 16; y++){
      for (int x = 8; x >= 0; x--){
        matrix.drawPixel(x, y+72, ang_eyes[y] & (1 << x));
        matrix.drawPixel(x, 63-y, ang_eyes[y] & (1 << x));
      }
    }
    matrix.write(); 
  }
